...
// 1. Move static data OUTSIDE the component to prevent memory recreation on re-renders
const SERVICES_DATA = [
    {
        icon: Leaf,
...
// 2. Extract into a React.memo Atomic Component (just like FeatureCard)
const ServiceCard = memo(({ icon: Icon, title, description }) => (
    <div className="p-8 rounded-[2rem] border border-white/40 bg-white/60 backdrop-blur-md shadow-[0_8px_32px_0_rgba(31,38,135,0.1)] text-center flex flex-col items-center transition-all duration-300 hover:shadow-[0_8px_32px_0_rgba(31,38,135,0.2)] hover:-translate-y-2">
        <div className="w-20 h-20 rounded-full bg-white/70 border border-white flex items-center justify-center mb-6 shadow-inner">
            <Icon className="w-10 h-10 text-teal-500" aria-hidden="true" />
        </div>
...
const Services = () => {
    ...
    return (
        // 3. Use semantic <motion.section> instead of <motion.div>
        <motion.section
            id="services"
            ref={ref}
            ...
            <div className="px-6 py-20 pb-32 max-w-6xl mx-auto">
                <div className="text-center mb-16">
                    <span className="inline-block py-2 px-6 rounded-full bg-teal-50 border border-teal-100 mb-4">
                        <h2 id="services-heading" className="text-teal-700 text-sm md:text-base font-bold tracking-widest uppercase m-0">
                            Our Services
                        </h2>
                    </span>
