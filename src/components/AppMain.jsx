    ...
    const loadAll = async () => {
        ...
        try {
            ...
            setProducts(products.map(p => ({
                id: p.id,
                name: p.name,
                price: parseFloat(p.price) || 0, // Ensure price is a number
                category: p.category || 'N/A', // Add category
                stock: parseInt(p.stock, 10) || 0 // Add stock
            })));
            ...
            setCustomers(customers.map(c => ({
                id: c.id,
                name: c.name,
                email: c.email || 'N/A', // Use email
                phone: c.phone || 'N/A', // Use phone
            ...
            setSales(sales.map(s => ({
                id: s.id,
                saleTimestamp: new Date(s.saleTimestamp || s.created_at), // Convert saleTimestamp to a Date object
                totalAmount: parseFloat(s.totalAmount) || 0, // Ensure totalAmount is a number
                customerId: s.customerId,
                customerName: s.customerName || 'N/A',
                items: s.items || [], // Ensure items array exists
                paymentMethod: s.paymentMethod || 'N/A', // Add paymentMethod
                status: s.status || 'Completed' // Add status
            })));
